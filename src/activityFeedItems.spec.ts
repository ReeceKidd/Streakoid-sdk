...
describe('SDK activityFeedItems', () => {
    const getRequestActivityFeed = jest
        .fn()
    ...
    describe('getAll', () => {
        ...
        test('calls GET with correct URL when no query parameters are passed', async () => {
            ...
            expect(getRequestActivityFeed).toBeCalledWith({ route: `/v1/activity-feed-items?limit=${limit}&` });
        });
        ...
        test('calls GET with correct URL when all query parameters are passed', async () => {
            ...
            expect(getRequestActivityFeed).toBeCalledWith({
