...
describe("POST /complete-solo-streak-tasks", () => {
  ...
  afterAll(async () => {
    ...
    await Promise.all(
      completeSoloStreakTasks.map((completeSoloStreakTask: any) => {
  ...
  describe("POST /v1/complete-solo-streak-tasks", () => {
    test("user can say that a solo streak task has been completed for the day", async () => {
      ...
      expect(completeSoloStreakTask.taskCompleteTime).toEqual(
        expect.any(String)
      );
      expect(completeSoloStreakTask.taskCompleteDay).toEqual(
        expect.any(String)
      );
